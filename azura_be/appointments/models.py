...
class Appointment(BaseModel):
    ...
    location = models.ForeignKey(
        Location, related_name="appointments", on_delete=models.PROTECT
    )
    ...
    status = models.CharField(
        max_length=50, choices=AppointmentStatusChoices, blank=True
    )
    completed_at = models.DateTimeField(null=True, blank=True)

    def save(self, *args, **kwargs):
        if (
            self.status == AppointmentStatusChoices.COMPLETED.name

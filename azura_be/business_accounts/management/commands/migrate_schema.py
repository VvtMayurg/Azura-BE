...
from django_tenants.utils import get_tenant_migration_order
...
class MigrateSchemasCommand(SyncCommon):
    ...
    def handle(self, *args, **options):
        ...
        executor = GET_EXECUTOR_FUNCTION(codename=self.executor)(
        ...
        if self.sync_tenant:
            if self.schema_name and self.schema_name != self.PUBLIC_SCHEMA_NAME:
                if not schema_exists(
                    self.schema_name, self.options.get("database", None)
                ):
                ...
                if has_multi_type_tenants():
                    ...
                    tenants = (
                        get_tenant_model()
                        .objects.only("schema_name", type_field_name)
                ...
                if has_multi_type_tenants():
                    ...
                    tenants = (
                        get_tenant_model()
                        .objects.filter(initial_completed=True)
                        .only("schema_name")
                        .exclude(schema_name=self.PUBLIC_SCHEMA_NAME)
